    ...
    <header style="widows: 1; -webkit-app-region: drag;"
        ...
        <div style="widows: 2; -webkit-app-region: no-drag;" class="flex items-center gap-2 pl-2">
            <button type="button" @click="isSettingsOpen = true"
                class="relative inline-flex h-8 w-8 items-center justify-center rounded-md text-[#6272A4] transition-colors hover:bg-[#44475A] hover:text-[#F8F8F2]"
                title="Settings">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                ...
                <span v-if="hasUpdateAvailable" class="absolute top-1.5 right-1.5 flex h-2.5 w-2.5">
                  <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#50FA7B] opacity-75"></span>
                  <span class="relative inline-flex rounded-full h-2.5 w-2.5 bg-[#50FA7B]"></span>
                </span>
            </button>
